...
# runtime-configuration Release Script
...
# Functions
print_usage() {
    echo -e "${BLUE}runtime-configuration Release Script${NC}"
    echo
...
get_latest_version() {
    git tag --sort=-version:refname | grep -E '^runtimeconfig/[0-9]+\.[0-9]+\.[0-9]+$' | head -n1 | sed 's/^runtimeconfig\///' || echo "1.0.0"
}
...
print_info "Monitor progress at: https://github.com/modelingevolution/runtime-configuration/actions"
